...
4. Use prefecture-level cities in the title and location. Honor the plugin's station alias result, including 温州南、平阳、瑞安 → 温州.
5. Reconcile only events containing an OpenClaw 12306 marker. Never modify an unrelated calendar event based only on similar title, station, or date.
6. Purchase creates or idempotently updates the marked itinerary. Refund deletes the exactly marked itinerary. Change updates a single reconciled itinerary; if zero or multiple candidates exist, require review rather than guessing.
7. For an interactive Feishu request, summarize the exact create/update/delete and obtain confirmation unless that message already explicitly authorizes it. The scheduled worker may bypass per-item confirmation only under its stored, bounded approval policy.
